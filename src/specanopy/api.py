...
from specanopy.graph import SpecGraph, build_graph, impact_summary
...
def serve_ui(specs_dir: Path, port: int = 8000, open_browser: bool = True):
    ...
    # If the UI hasn't been built yet, we can serve a placeholder or raise an error
    import os
    if not ui_dist_dir.exists():
        os.makedirs(ui_dist_dir, exist_ok=True)
        (ui_dist_dir / "index.html").write_text(
            "<html><body><h1>Specanopy UI not built yet.</h1><p>Run npm run build in the ui/ directory.</p></body></html>"
        )
        
    handler = lambda *args, **kwargs: GraphUIHandler(
        *args, directory=str(ui_dist_dir), specs_dir=specs_dir, **kwargs
    )
